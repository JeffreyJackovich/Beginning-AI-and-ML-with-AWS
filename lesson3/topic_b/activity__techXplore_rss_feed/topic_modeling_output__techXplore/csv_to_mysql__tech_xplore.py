...
with conn.cursor() as cur:
	for row in row_list:
		cur.execute("CREATE TABLE IF NOT EXISTS tech_xplore_doc_topics (id INTEGER NOT NULL AUTO_INCREMENT, docname VARCHAR(256) NOT NULL, \
														topic INTEGER NOT NULL, \
														proportion FLOAT NOT NULL,\
									                    PRIMARY KEY (id) );") 
		cur.execute("INSERT IGNORE INTO linkedTopicDB2.tech_xplore_doc_topics (docname, topic, proportion) VALUES (%s, %s, %s)", (row[0], row[1], row[2]))
	conn.commit()
	conn.close()